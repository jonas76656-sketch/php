...
// --- Server-Side API Logic (POST handling) ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['login_key'])) {
    ...
    if (!$pm) { $err = $res1['error']['message'] ?? "Tokenization Error"; echo json_encode(["status" => "DEAD", "msg" => "$err $bin_info"]); exit; }
...
<div class="wrapper">
    ...
    <select id="gate" class="gate-select"><option value="gate1">⚡ GATE 1: STRIPE $1.00 (Charge)</option><option value="gate2">⚡ GATE 2: STRIPE $0.50 (Auth)</option></select>
