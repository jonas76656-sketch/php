...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // User Agent ကို Desktop ပုံစံပြောင်းလဲထားခြင်း (ပိုမိုတည်ငြိမ်စေရန်)
    ...
    // --- Step 1: Tokenization with Improved Headers ---
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { background: var(--bg); color: var(--text); font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 20px; margin: 0; display: flex; justify-content: center; }
        .wrapper { width: 100%; max-width: 900px; animation: fadeIn 0.8s ease; }
        @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        
        .header-box { text-align: center; background: linear-gradient(90deg, #161b22, #0d1117); padding: 30px; border-radius: 15px; border: 1px solid var(--border); margin-bottom: 25px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
        h1 { font-size: 1.8rem; color: var(--accent); margin: 0; text-transform: uppercase; letter-spacing: 2px; }
        ...
        .upload-label { position: absolute; top: 15px; right: 15px; background: #30363d; color: #c9d1d9; border: 1px solid var(--border); padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 12px; font-weight: bold; transition: 0.3s; }
        .upload-label:hover { background: var(--accent); color: #fff; }
        
        .controls { display: flex; gap: 15px; margin: 25px 0; }
        #btn { flex: 2; background: linear-gradient(45deg, #238636, #2ea043); color: white; border: none; padding: 18px; border-radius: 12px; cursor: pointer; font-weight: bold; font-size: 1.1rem; transition: 0.3s; box-shadow: 0 10px 20px rgba(35, 134, 54, 0.2); }
        #stopBtn { flex: 1; background: #da3633; color: white; border: none; padding: 18px; border-radius: 12px; cursor: pointer; font-weight: bold; display: none; transition: 0.3s; }
        #btn:hover { transform: translateY(-2px); box-shadow: 0 15px 25px rgba(35, 134, 54, 0.3); }

        .stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 15px; margin-bottom: 25px; }
        .stat-box { background: var(--card); border: 1px solid var(--border); padding: 15px; border-radius: 15px; text-align: center; transition: 0.3s; position: relative; overflow: hidden; }
        .stat-box:hover { border-color: var(--accent); transform: translateY(-3px); }
        .stat-box small { font-size: 11px; color: #8b949e; text-transform: uppercase; display: block; margin-bottom: 5px; }
        .stat-box span { font-size: 22px; font-weight: bold; display: block; }
        
        .result-box { background: var(--card); border: 1px solid var(--border); border-radius: 15px; margin-bottom: 15px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
        .res-head { padding: 15px 20px; font-weight: bold; font-size: 14px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.02); transition: 0.3s; }
        ...
        .LIVE { color: #3fb950; text-shadow: 0 0 10px rgba(63, 185, 80, 0.3); } 
        .INSUF { color: #d29922; } 
        .CVV { color: #58a6ff; } 
        .DEAD { color: #f85149; }
...
<div class="wrapper">
    ...
    <select id="gate" class="gate-select">
        <option value="gate1">🛡️ GATE 1: STRIPE $1.00 ( texas-don )</option>
        <option value="gate2">🛡️ GATE 2: STRIPE $0.50 ( chris-don )</option>
    </select>

    <div class="input-group">
        <textarea id="list" placeholder="Paste cards here... (Format: CC|MM|YY|CVV)"></textarea>
        <label for="fileInput" class="upload-label"><i class="fa-solid fa-file-import"></i> IMPORT .TXT</label>
    ...
    <div class="result-box"><div class="res-head" style="color:#d29922" onclick="toggleBox('l_insuf')"><span><i class="fa-solid fa-money-bill-wave"></i> INSUFFICIENT FUNDS</span> <i class="fa-solid fa-chevron-down"></i></div><div class="res-body" id="l_insuf"></div></div>
    <div class="result-box"><div class="res-head" style="color:#58a6ff" onclick="toggleBox('l_cvv')"><span><i class="fa-solid fa-shield-halved"></i> 3Ds / CCN LIVE</span> <i class="fa-solid fa-chevron-down"></i></div><div class="res-body" id="l_cvv"></div></div>
...
<script>
    ...
    function stop() { 
        ...
        document.getElementById('btn').innerHTML = '<i class="fa-solid fa-rotate-right"></i> RESUME SCANNING';
    ...
    async function start() {
        ...
        while (lines.length > 0 && isRunning) {
            ...
            try {
                ...
                item.innerHTML = `<span class="${data.status}">[${data.status}]</span> ${line} -> <span class="${data.status}">${data.msg}</span> <span class="taken-time">[${timeTaken}s]</span>`;
        ...
        if (lines.length === 0) {
            statusBox.innerText = "SCANNING FINISHED.";
